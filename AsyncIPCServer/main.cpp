// AsyncIPCServer.cpp : 定义应用程序的入口点。
//

#include "stdafx.h"
#include "AsyncIPCServer.h"
#include <ShlObj.h>
...
public:
    void OnCreate(bool) override {
        ...
        wcscat_s(buffer, L"\\AsyncIPCClient.exe");
        STARTUPINFO si = { sizeof(STARTUPINFO) };
        PROCESS_INFORMATION pi = { 0 };
        if (!::CreateProcess(NULL, (LPWSTR)buffer, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
                     ...
                     _In_ int       nCmdShow)
{
    ...
    InitEasyLog("server");
    // LOG(INFO) << "AsyncIPC server start";
 	// TODO:  在此放置代码。
	MSG msg;
	HACCEL hAccelTable;
	...
	LoadString(hInstance, IDC_ASYNCIPCSERVER, szWindowClass, MAX_LOADSTRING);
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ASYNCIPCSERVER));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_ASYNCIPCSERVER);
